...
public class BoardArray implements Board
{
			...
			for (int j=i; j < i+8; j++)
			{
				...
				else
				{
					switch (p)
					{
						...
						case 'p':
							cb[j] = new Pawn(BLACK);
							System.err.println(j + " is Black uk.ac.gla.chessmantis.piece.Pawn");
							break;
						case 'R':
							cb[j] = new Rook(WHITE);
							System.err.println(j + " is White uk.ac.gla.chessmantis.piece.Rook");
							break;
						case 'N':
							cb[j] = new Knight(WHITE);
							System.err.println(j + " is White uk.ac.gla.chessmantis.piece.Knight");
							break;
						case 'B':
						...
						case 'Q':
							...
							System.err.println(j + " is White uk.ac.gla.chessmantis.piece.Queen");
							break;
						case 'K':
							cb[j] = new King(WHITE);
							System.err.println(j + " is White uk.ac.gla.chessmantis.piece.King");
							break;
						case 'P':
							cb[j] = new Pawn(WHITE);
							System.err.println(j + " is White uk.ac.gla.chessmantis.piece.Pawn");
							break;
					}
	...
	/* (non-Javadoc)
	 * @see uk.ac.gla.chessmantis.Board#isCheckmate()
	 */
	...
	public boolean isInCheck(boolean colour, int post, int posf)
	{
		...
		/* TT Position of the uk.ac.gla.chessmantis.piece.King corresponding to
	...
	/* (non-Javadoc)
	 * @see uk.ac.gla.chessmantis.Board#isStalemate()
	 */
	 ...
	 * modifier
	 * @author TT & DG
	 * @param m uk.ac.gla.chessmantis.Moveable m. absolute move on a 64 square board
	...
	public boolean makeMove(Moveable m) // DG Might be and is necessary to shift checks for kings' positions to inside isLegalmove, will do that on friday
	{
		...
		if (isCastling(m))
		{
			if (canCastle(m))
			{
				...
				// uk.ac.gla.chessmantis.Move the rook
		...
		cb[post].setMoved();	// DG uk.ac.gla.chessmantis.piece.Piece has moved, so increment move counter
	...
	public boolean reverseMove(Moveable m)
	{
		...
		if (wasCastling(m))
		{
			...
			else
			{
				if (cb[post-1] instanceof Rook)
				{
				// uk.ac.gla.chessmantis.piece.Rook's on the left
				// Shift him back
				cb[post+1] = cb[post-1];
				cb[post-1] = null;

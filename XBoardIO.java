...
public class XBoardIO implements Readable, Writeable, Runnable
{
	...
	/** Writes a MoveEvent to the XBoard interface. */
	...
	/** Writes a StatusEvent to the XBoard interface. */
	...
	/** Writes an IllegalMoveEvent to the XBoard interface. */
	...
	/** Writes an ErrorEvent to the XBoard interface. */
	...
	/** Returns the next ChessEvent, or null if no event found. */
	...
	/**
 	 * returns an ChessEvent or null if it needs to recieve the next command before returning a ChessEvent
 	 *
 	 * @param	command	A string containing the command and any arguments it carried
 	 * @return		A ChessEvent or null
 	 * @see			ChessEvent
 	 */
	private ChessEvent parseCommand(String commandline)
	{
		...
		else if(command.equals("result"))
		{
			...
			if(token.equals("1-0"))
			{
				System.err.println("XBoardIO Win");
